...
export default defineConfig({
  ...
  shortcuts: {
    ...
    // A large series count (esp. per-session labels) must scroll in its own
    // side column rather than shrink the plot area next to it — see usage-chart.ts.
    "chart-legend":
      "flex flex-col gap-0.5 w-56 shrink-0 min-h-0 overflow-y-auto border-l border-border pl-3",
    "chart-legend-item":
      "flex items-center gap-1.5 w-full bg-transparent border-none rounded-[4px] px-1.5 py-1 text-[0.8rem] text-fg text-left cursor-pointer hover:bg-hover focus-visible:[outline:2px_solid_var(--focus-ring)] focus-visible:[outline-offset:-2px]",
    "chart-legend-item-hidden": "opacity-50 line-through",
    "chart-legend-swatch": "w-2.5 h-2.5 rounded-[2px] shrink-0",
    "chart-legend-label": "flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap",
